...
/**
 * Render the Vehicle Purchase Agreement.
 *
  ...
  return new Promise((resolve, reject) => {
    const doc = new PDFDocument({
      ...
      info: {
        ...
        Author: B.legalName,
        Subject: `Vehicle Purchase Agreement for ${agreement.item_title}`,
...
function renderTitle(doc, agreement) {
  doc.fontSize(14).font('Helvetica-Bold').fillColor(INK)
    .text(`VEHICLE PURCHASE AGREEMENT #${agreement.agreement_number}`,
...
function renderObjectOfSale(doc, a) {
  ...
  dataTable(doc, [
    ...
    ['Odometer', a.item_meter],
    ['Engine HP', a.item_engine_hp],
    ['Vehicle Identification Number (VIN)', a.item_vin],
...
function renderOwnershipTransfer(doc, a) {
  ...
  richBody(doc,
    `**${a.seller_name}** certifies that it is the lawful owner of the above-described Vehicle ` +
    'and that it is free of all encumbrances, liens and any and all legal claims. Parties agree ' +
    'to sign all documents necessary to transfer title and ownership of the Vehicle from the ' +
    `Seller to the Buyer within ${B.terms.inspectionDays} days of the date of this Agreement.`);
  body(doc,
    'The Seller shall provide the title or equivalent ownership document for the Vehicle. The '
    + 'Buyer shall be liable for all administrative costs relating to titling and registration of '
    + 'the Vehicle in his or her name, together with any state inspection or emissions certificate '
    + 'required in the jurisdiction where the Vehicle is registered.');
}
...
function renderBuyBack(doc) {
  const t = B.terms;
  sectionHeading(doc, 6, 'Vehicle Buy Back Guarantee');
  richBody(doc,
  ...
  richBody(doc,
    'The purchased vehicle must not be altered, repaired, resold or used commercially, and must ' +
    'not be driven beyond what is required to verify its condition — not more than ' +
    `**${t.maxInspectionMiles} miles** from the odometer reading at time of sale. In the event of ` +
    'a return, the Buyer is NOT responsible for freight and handling charges. In the event of a ' +
    `return, the Seller agrees to refund the Buyer, in full, within ${t.refundWindowHours} hours.`);

  richBody(doc,
    `If the Buyer decides to return the Vehicle, the Seller (**${B.legalName}**) agrees to ` +
