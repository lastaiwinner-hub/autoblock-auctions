...
function parseListingForm(body) {
  ...
  return {
    ...
    mileage: body.mileage === '' || body.mileage === undefined ? null : Number(body.mileage),
    vin: String(body.vin || '').trim().toUpperCase() || null,
    ...
    drivetrain: String(body.drivetrain || '').trim() || null,
    fuel_type: String(body.fuel_type || '').trim() || null,
    body_style: String(body.body_style || '').trim() || null,
    exterior_color: String(body.exterior_color || '').trim() || null,
    interior_color: String(body.interior_color || '').trim() || null,
    doors: body.doors === '' || body.doors === undefined ? null : Number(body.doors),
    seats: body.seats === '' || body.seats === undefined ? null : Number(body.seats),
    title_status: String(body.title_status || 'Clean').trim(),
...
function validateListing(data, excludeId = null) {
  ...
  // A VIN is printed on the purchase agreement and used for titling, so a
  // malformed one is worth catching before the listing goes live.
...
router.post('/orders/:id/status', asyncRoute(async (req, res, next) => {
  ...
  if (status === 'invoiced') {
    ...
    req.flash('success', 'Buyer notified that the vehicle is in transit.');
  } else {
