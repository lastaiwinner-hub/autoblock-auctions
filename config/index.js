...
const writableRoot = onVercel ? '/tmp/autoblock' : root;
...
if (onVercel) {
  ...
  const liveFile = str('DATABASE_FILE', path.join(dirs.data, 'autoblock.sqlite'));
  try {
...
module.exports = {
  ...
  db: {
    file: str('DATABASE_FILE', path.join(dirs.data, 'autoblock.sqlite')),
  },
  ...
  session: {
    ...
    name: str('SESSION_COOKIE_NAME', 'autoblock.sid'),
  ...
  admin: {
    ...
    email: str('ADMIN_EMAIL', 'admin@autoblock-auctions.com'),
